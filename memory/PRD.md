...
## Implemented (2026-09-21c) — Banner pengumuman + Harga/Promo admin
- Banner Pengumuman di halaman klien: Header.js fetch `GET /api/settings/public` → banner gelap (ikon Megaphone) tampil di paling atas semua halaman pengguna (Dashboard/Akun/Harga/Proyek/dll) bila `announcement` tidak kosong. Catatan admin di AdminSettings diperbarui (bukan lagi "untuk kebutuhan mendatang").
- Harga & Promo dapat diubah admin: settings tambah field monthlyPrice/monthlyPromo/yearlyPrice/yearlyPromo/promoActive (default 149000/149000/1290000/1290000/false). `GET/PUT /api/admin/settings` (admin-only) terima field ini; `GET /api/settings/public` mengeksposnya. AdminSettings.js: kartu "Harga & Promo" (toggle Aktifkan Harga Promo + 4 input angka per paket Bulanan/Tahunan + hint efektif & % diskon). Pricing.js: harga dinamis dari settings — harga normal dicoret + harga promo + badge "PROMO SPESIAL" saat promo aktif; hemat tahunan dihitung dinamis. Checkout `POST /api/subscription/checkout` hitung gross_amount = _effective_price(base, promo, promoActive) (promo dipakai hanya bila aktif & 0<promo<base). 
- Terverifikasi testing agent 4/4: settings/public ekspos field + default benar; admin guard 403 utk user biasa (GET+PUT); checkout monthly gross_amount=149000 & yearly=1290000 (dari settings), tanpa 500. Visual: banner + harga promo (Rp149.000→Rp99.000, Rp790.000/th hemat 34%) terverifikasi via screenshot (data uji sudah di-reset).

## Implemented (2026-09-21d) — Countdown promo + warna banner
- Countdown Promo: settings tambah `promoEndsAt` (ISO, kosong=tanpa batas) + `announcementTheme` (info/promo/warning, divalidasi pydantic Literal). `_promo_active()` di backend: promo nonaktif otomatis setelah promoEndsAt (checkout kembali kenakan harga normal). `/api/settings/public` tambah promoEndsAt+announcementTheme+`serverNow` (koreksi jam klien). Frontend Pricing.js: timer hitung mundur live ("Berakhir dalam X hari HH:MM:SS") saat promo aktif & berbatas waktu; harga normal otomatis pulih saat promo berakhir. AdminSettings.js: input datetime-local "Promo Berakhir Pada" + 3 pilihan warna banner (Info biru / Promo gelap / Peringatan merah) dgn preview garis warna. Header.js: banner pengumuman warna dinamis per tema.
- Terverifikasi testing agent 4/4: settings/public ekspos field baru+serverNow; promo kedaluwarsa→checkout gross_amount=350000 (harga normal); promo aktif→gross_amount=149000 (harga promo); settings owner DIPULIHKAN persis dari snapshot. Visual: banner merah (warning) + countdown berdetak terverifikasi screenshot; settings owner aman & utuh.
- CATATAN: owner sudah memakai fitur ini live (pengumuman "Promo Launching WEB ProFinance Interior 50%", harga 350.000→149.000 dst). JANGAN reset settings owner.
